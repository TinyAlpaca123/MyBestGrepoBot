...
class GrepoBot {
    ...
    async RunBot() {
        try {
            ...
            let massRecruit={};
            let groups=us.map(this.Config.Groups,tasks=>{return{towns:ITowns.town_groups.find(x=>x.attributes.name==tasks.GroupName)?.towns,tasks:tasks}}).filter(x=>x.towns!=undefined);
            let doRecruit=false;
